...
$result = mysql_query("SELECT `campus_schedule`.*  FROM `campus_schedule` where ".$sql." and status=1 and `campus_schedule`.std_status!=3 and `campus_schedule`.std_status!=4 and `campus_schedule`.std_status!=7 and teacherID IN ('".getTeam($_SESSION['userId'])."') and classType in (".getPlan($_POST['days'][0]).")  ") or trigger_error(mysql_error());
}
else{
...
if($_POST['getTL_ALL']==1)
{
	...
	{
		$id_getTL[]=$row_getTL['id'];
		
	}
	//echo implode("','",$id);
	$all_getTL = implode("','",$id_getTL);
	echo "<br>";echo "<br>";
	...
	{
		$id_getTeachers[]=$row_getTeachers['id'];
		
	}
	//echo implode("','",$id);
	$all_getTeachers = implode("','",$id_getTeachers);
	$result = mysql_query("SELECT `campus_schedule`.* FROM `campus_schedule` where ".$sql." and status=1 and `campus_schedule`.std_status!=3 and `campus_schedule`.std_status!=4 and `campus_schedule`.std_status!=7 and teacherID IN ('".$all_getTeachers."') and classType in (".getPlan($_POST['days'][0]).") and startDate<='".prepareDate($_POST['classDate'])."' and endDate>='".prepareDate($_POST['classDate'])."' ") or trigger_error(mysql_error());
...
$result = mysql_query("SELECT `campus_schedule`.* FROM `campus_schedule` where ".$sql." and status=1 and `campus_schedule`.std_status!=3 and `campus_schedule`.std_status!=4 and `campus_schedule`.std_status!=7 and teacherID IN ('".getTeam($_POST['TeacherTL'])."') and classType in (".getPlan($_POST['days'][0]).") and startDate<='".prepareDate($_POST['classDate'])."' and endDate>='".prepareDate($_POST['classDate'])."' ") or trigger_error(mysql_error());
}
