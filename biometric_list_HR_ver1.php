...
else 
{
	...
	if($TS_CI>$onDuty_plus_10min && $TS_CI<=$onDuty_plus_1hr)
	{
		...
		$late_count_MOD= $late_count % 4;
		if($late_count_MOD==0)
		{
			$late_count_output=$late_count_output+1;
		}
	}
...
else 
{
	...
	else
	{
		...
		$earlyOut_count=$earlyOut_count+1;
		$earlyOut_count_MOD= $earlyOut_count % 4;
		if($earlyOut_count_MOD==0)
		{
			$earlyOut_count_output=$earlyOut_count_output+1;
		}
		}
...
$_8hrs = 28800;
												/*c)Output <5 absent*/
if(($row['clockIn']=='' && $row['clockOut']=='') || $Output<$_5hrs)
{ 
...
else
{
	...
	else if($Output>=$_5hrs && $Output<$_8hrs){
		echo "<td valign='top' style='color:purple; font-weight:bold;'>SHORT LEAVE DUTY HOURS</td>";
		/***************COUNTING SHORT LEAVE DUTY HOURS******************/	//Start
			$shortLeave_count_DUTY_HOURS=$shortLeave_count_DUTY_HOURS+1;
			$shortLeave_count_DUTY_HOURS_MOD= $shortLeave_count_DUTY_HOURS % 2;
			if($shortLeave_count_DUTY_HOURS_MOD==0)
			{
				$shortLeave_count_DUTY_HOURS_output=$shortLeave_count_DUTY_HOURS_output+1;
			}
		/***************COUNTING SHORT LEAVE DUTY HOURS******************/	//End
	}
...
echo "<td style='color:green; font-weight:bold'>Casual Leave:</td>";
	...
	FROM campus_empleave WHERE LeaveType=2 ";
	...
	$row_leave_app_CL['EmpID'];
	echo $days_count = $row_leave_app_CL['NoOfDays'];
	}
	$rowcount_leave_app_CL = mysql_num_rows($result_leave_app_CL);	//ROW COUNT
//Applied but NOT APPROVED by GM
if($row_leave_app_CL['GMApprove']!=1 && $rowcount_leave_app_CL>=1){
echo "<td style='color:BLUE; font-weight:bold'>".$rowcount_leave_app_CL."(NOT APPROVED BY GM)</td>";
}
//NOT APPLIED at all
if($rowcount_leave_app_CL==0){
...
echo "<td style='color:green; font-weight:bold'>Sick Leave:</td>";
	...
	FROM campus_empleave WHERE LeaveType=1 ";
	if($_SESSION['userType']==3)
	{	
	...
	while($row_leave_app_SickLeave =  mysql_fetch_array($result_leave_app_SickLeave)){
	$row_leave_app_SickLeave['EmpID'];
	echo $days_count_SickLeave = $days_count_SickLeave+$row_leave_app_SickLeave['NoOfDays'];
	}
...
//NOT APPLIED at all
if($rowcount_leave_app_SickLeave==0){
...
/*********** Table for late,earlyOut,shortLeave DEDUCTIONS **************/	//START
echo "<table  border=1 id='' cellspacing=0 >"; 
/********** late_count_output **********/ //START
echo "<tr>";
echo "<td style='color:green; font-weight:bold'>Late Count(4 Late = 1 day deduction):</td>";
echo "<td style='color:green; font-weight:bold'>".$late_count_output."</td>";
echo "</tr>";
...
$TOTAL_PRESENT_DAYS = $TOTAL_PRESENT_DAYS - ($late_count_output+$earlyOut_count_output+$shortLeave_count_output+$shortLeave_count_DUTY_HOURS_output);
