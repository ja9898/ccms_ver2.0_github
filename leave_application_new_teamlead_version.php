...
<div id="label">Team:</div><div id="field"><?php echo getDataList_leave_application_new_teamlead_version('','teacher_id',3,$_SESSION['userId']);?><?php
getFilterSubmit();
?></div>
...
echo $seven_days_range = floor($seven_days_range/(60*60*24));
			...
			if($systemdate>=prepareDate($_POST['LeaveStartDate']) && $seven_days_range<7)
			{
				if($_POST['casual_leave']==1)
				{	
					$result_CL_dual=mysql_query("SELECT * FROM campus_empleave WHERE EmpID='".$_POST['teacher_id']."'
					AND LeaveType=2");
...
//Used $_POST['teacher_id'] instead of $_SESSION['userId'] as TL has to apply for leave on behalf of teacher
if(isset($_POST['teacher_id'])){
$teacherID = $_POST['teacher_id'];
$sql_employee_info=("SELECT * FROM capmus_users WHERE id='".$_POST['teacher_id']."' ");
$sql_employee_info_result=mysql_query($sql_employee_info) or die(mysql_error());
$sql_employee_info_result=mysql_fetch_array($sql_employee_info_result);
