...
echo " <a href='daily_scheduler_assessment_report_dialog.php'>DAILY SCHEDULER-CONFIDENTIAL</a> ";
echo "<br>" ;?>
...
{ 
	echo "<label>UPLOAD DATE:</label>";
	echo getInput(stripslashes($_POST['assess_date']),'assess_date','class=flexy_datepicker_input'); 
}
?>
...
if($_SESSION['userId']==185 || $_SESSION['userId']==251 || $_SESSION['userId']==136 || $_SESSION['userId']==426 || $_SESSION['userId']==97 || $_SESSION['userId']==420 || $_SESSION['userId']==448 || $_SESSION['userType']==1)
{
...
//echo "<br>";
$systemdate_day = date('d', strtotime(nl2br($systemdate)));
//echo "<br>";
$seven_days_ahead = date('Y-m-d', strtotime(nl2br($systemdate). ' + 7 days'));
//echo "<br>";
$seven_days_ahead_day = date('d', strtotime(nl2br($systemdate). ' + 7 days'));
...
$systemdate_day = date('d', strtotime(nl2br($systemdate)));
//echo "<br>";
$five_days_ahead = date('Y-m-d', strtotime(nl2br($systemdate). ' + 5 days'));
//echo "<br>";
$five_days_ahead_day = date('d', strtotime(nl2br($systemdate). ' + 5 days'));
...
$result=mysql_query("SELECT campus_schedule.id,campus_schedule.duedate as due_date,campus_schedule.paydate,
	...
	campus_schedule.assessment_filepath,campus_schedule.assessment_date,
	month(campus_schedule.assessment_date) as assess_month  
...
else if(!empty($_POST['assess_date']))
	{
		$result=mysql_query("SELECT campus_schedule.id,campus_schedule.duedate as due_date,campus_schedule.paydate,
	day(campus_schedule.duedate) AS dayz,month(campus_schedule.duedate) AS month,year(campus_schedule.duedate) AS year,
	day(campus_schedule.paydate) AS payday,month(campus_schedule.paydate) AS paymonth,year(campus_schedule.paydate) AS payyear,
	...
	month(campus_schedule.assessment_date) as assess_month  
	FROM campus_schedule 
	where campus_schedule.assessment_date='".prepareDate($_POST['assess_date'])."' ") 
	or trigger_error(mysql_error());
	}
...
//Actual PAY DATE
$pay_DATE = date('Y-m-d', strtotime(nl2br($row['paydate'])));
/////////////////
//Adding 15 days in PAY DATE
$pay_DATE_plus_15 = date('Y-m-d', strtotime(nl2br($pay_DATE). ' + 15 days'));
////////////////
...
$complete_curr_paydate_plus_15 = date('Y-m-d', strtotime( $current_year_curr_plus_15."-".$current_month_curr_plus_15."-".$paydate_date_curr_plus_15));
///////////////////////////////////////////
//Subtracting PAY DATE and PAY DATE PLUS 15 to get 15 DAYS 
$complete_curr_paydate_sub_15 = abs(strtotime($pay_DATE) - strtotime($pay_DATE_plus_15));
...
$days_15    = floor(($complete_curr_paydate_sub_15 - $years * 365*60*60*24 - $months*30*60*60*24)/ (60*60*24));
//////////////////////////////////////
...
$complete_curr_paydate_plus_5 = date('Y-m-d', strtotime( $current_year_curr_plus_5."-".$current_month_curr_plus_5."-".$paydate_date_curr_plus_5. ' + 5 days'));


//Subtracting PAY DATE PLUS 5 and PAY DATE PLUS 15 to get 5 DAYS
$complete_curr_paydate_sub_5 = abs(strtotime( $complete_curr_paydate_plus_5) - strtotime($complete_curr_paydate_plus_15));
...
$days_5    = floor(($complete_curr_paydate_sub_5 - $years * 365*60*60*24 - $months*30*60*60*24)/ (60*60*24));


if($_SESSION['userId']==185 || $_SESSION['userId']==251 || $_SESSION['userId']==136 || $_SESSION['userId']==426 || $_SESSION['userId']==97 || $_SESSION['userId']==420 || $_SESSION['userId']==448 || $_SESSION['userType']==1)
{ 
	//IF paydate + 15 days and the PINK blinking will be UP between 5 days
	if(($pay_DATE!='1970-01-01') && ($days_15 == 15) && ($days_5 == 5) && ($systemdate >= $complete_curr_paydate_plus_15	) && ($systemdate <= $complete_curr_paydate_plus_5	) && ($row['assess_month']!=date('n')))
	{
		echo "<td valign='top' class='myDiv'>PENDING REPORT</td>";	
	}
	//IF paydate + 15 days and the BLUE blinking will be UP for more than 5 days gone
	else if(($pay_DATE!='1970-01-01') && ($days_15 == 15) && ($days_5 == 5) && ($systemdate >= $complete_curr_paydate_plus_15) && ($row['assess_month']!=date('n')))
	{
		echo "<td valign='top' class='myDivblue'>PENDING REPORT</td>";	
	}
	//IF paydate + 15 days and the Green TEXT will be SHOWN if the assessment is submitted within 5 days and in current month
	else if(($pay_DATE!='1970-01-01') && ($days_15 == 15) && ($days_5 == 5) && ($systemdate >= $complete_curr_paydate_plus_15	) && ($systemdate <= $complete_curr_paydate_plus_5	) && ($row['assess_month']==date('n')))
	{
		echo "<td valign='top' style='color:green; font-weight:bold'>SUBMITTED</td>";	
	}
	//IF condition of paydate + 15 days is NOT MET
	else
	{
