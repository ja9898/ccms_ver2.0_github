<?php
	define(SOFTPHONE_URL_RELOAD,'softphone.php');
	...
	//To check if the softphone page is accessed by the Authrozied logged in user only. No one can use someone else softphone page with his account.
	$unauth_flag = 1;
	
	/*
	if(isset($_SESSION['Admin_Id']) || $_SESSION['Admin_Id'] == $uid){
		$unauth_flag = 0;
	}
	...
	if($unauth_flag == 1){
		echo 'Invalid or Unauthorized Access. Please Relogin with your portal/ panel again and try again. Thank You!';
		exit;
		
	}//end if($unauth_flag == 1)
	*/
	if(isset($_GET['uid'])){
		
		//Get Employee Rercord
		$qry_user_data = "SELECT firstName, lastName, voice_id, v_password, sip_server FROM capmus_users WHERE Id = '".$uid."'";
		$rs_user_data = mysql_query($qry_user_data) or die(mysql_error());
		$row_user_data = mysql_fetch_array($rs_user_data);
		
	}//end if(isset($_GET['uid']))
	
	$server_url = trim($row_user_data['sip_server']);
	$username	= trim($row_user_data['voice_id']);
	$sip		= "sip:".$username."@".$server_url;
	$password   = trim($row_user_data['v_password']);
	$fullname   = ucwords(strtolower($row_user_data['firstName'].' '.$row_user_data['lastName']));
	//$fullname   = $username;
	$number		= (isset($_GET['number'])) ? trim($_GET['number']) : '';
	$releam  	= 'biralsabia';
	
	/*	
	$sip		= "sip:8000@115.186.163.171";
	$username	= "8000";
	$password   = "test786";
	$fullname   = "8000";
...
<html xmlns="http://www.w3.org/1999/xhtml" lang="en" xml:lang="en"> 
    <head>
		<title>Welcome to VBAS Dialer</title>
    ...
    <body>
		...
		?>
            ...
            <center>
            <p style="padding: 0px; margin: 0px;">Connected as <b>[<?php echo ucwords(strtolower($row_user_data['firstName'].' '.$row_user_data['lastName'])) ?>]</b></p>
            <div style="width:100%; float:left">
               ...
               <div style="float:left;width:385px;">
					...
					?>
							<a href="<?php echo SOFTPHONE_URL_RELOAD?>?uid=<?php echo $uid ?>&number=<?php echo CUSTOMER_SUPPORT_VOICEID ?>" title="Call Customer Support"><img src="assets/cutomersupport.png" width="26" border="0" alt="Call Customer Support" /></a>                    
                            &nbsp;&nbsp;
                            <a href="<?php echo SOFTPHONE_URL_RELOAD?>?uid=<?php echo $uid ?>&number=<?php echo SALES_SUPPORT_VOICEID ?>" title="Call Sales Support"><img src="assets/salesupports.png" width="26" border="0" alt="Call Sales Support" /></a>
                            &nbsp;&nbsp;
                    
                    <?php							
						}else{
					?>
                        <a href="<?php echo SOFTPHONE_URL_RELOAD?>?uid=<?php echo $uid ?>&number=<?php echo KITCHEN_SUPPORT_VOICEID ?>" title="Call Kitchen"><img src="assets/kitchen.png" width="16" border="0" alt="Call Kitchen" /></a>
                        &nbsp;&nbsp;
                        <a href="<?php echo SOFTPHONE_URL_RELOAD?>?uid=<?php echo $uid ?>&number=<?php echo IT_SUPPORT_VOICEID ?>" title="Call IT Support"><img src="assets/it_support.png" width="26" border="0" alt="Call IT Support" /></a>
                        &nbsp;&nbsp;
                    
                    <?php		
						}//end if(isset($_SESSION['customerLogin']))
                    ?>
                    
                 <a href="<?php echo SOFTPHONE_URL_RELOAD?>?uid=<?php echo $uid ?>&number=" title="ReConnect" style="margin-left:120px"><img src="assets/reload.png" width="26" border="0" alt="ReConnect" /></a>
               </div>
            </div>
			<?php
                if($number != ''){
            ...
            <div style="width:100%; float:left">
                        <a href="<?php echo SOFTPHONE_URL_RELOAD?>?uid=<?php echo $uid ?>&number=<?php echo '11'. substr($number,2,strlen($number)); ?>" title="<?php echo 'Call 11'.substr($number,2,strlen($number)); ?>">
						<strong>[<?php echo '11'.substr($number,2,strlen($number)); ?>]</strong></a>
                        &nbsp;&nbsp;
                        <a href="<?php echo SOFTPHONE_URL_RELOAD?>?uid=<?php echo $uid ?>&number=<?php echo '22'.substr($number,2,strlen($number)); ?>" title="<?php echo 'Call 22'.substr($number,2,strlen($number)); ?>"><strong>[<?php echo '22'.substr($number,2,strlen($number)); ?>]</strong></a>
                        &nbsp;&nbsp;
                        <a href="<?php echo SOFTPHONE_URL_RELOAD?>?uid=<?php echo $uid ?>&number=<?php echo '33'.substr($number,2,strlen($number)); ?>" title="<?php echo 'Call 33'.substr($number,2,strlen($number)); ?>"><strong>[<?php echo '33'.substr($number,2,strlen($number)); ?>]</strong></a>
            </div>
			<?php		
                }//end if($number != '')
            ?>
		...
		?>
        		<p style="height:100px; font-weight:bold; font-size:13px;"><u>Attention:</u>Please Open Your Portal/ Panel in "Google Chrome" Browser to Use this Feature. Thank You!</p>
        <?php				
			}//end if
			*/
		?>
		...
		function sipRegister() {
			...
			try {
				...
				oSipStack = new SIPml.Stack({
						...
						websocket_proxy_url: "ws://<?php echo $server_url?>:8088/ws",
						outbound_proxy_url: "",
						ice_servers: "",
