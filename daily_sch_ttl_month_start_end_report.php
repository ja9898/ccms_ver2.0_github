...
if(isset($_POST['send_email_month_start_report']))
{
	...
	$email_to_send_on_MONTH_START_REPORT = "<div align='center' style='color:Orange; font-size:20px; font-weight:bold'>Email From YOURCLOUDCAMPUS</div>
			...
			<tr bgcolor=#CD96CD>
			<td><b>Grade: </b></td>
			<td valign='top'> ".$sql_row_values['grade']."</td>
			</tr>
			
			<tr bgcolor=#EED2EE>
			<td><b>Teacher: </b></td>
			<td valign='top'> ".showUser( nl2br( $sql_row_values['teacherID']))."</td>
			</tr>
			
			<tr bgcolor=#CD96CD>
			<td><b>Course: </b></td>
			<td valign='top'> ".getData( nl2br( $sql_row_values['courseID']),'course')."</td>
			</tr>
			
			<tr bgcolor=#EED2EE>
			<td><b>Topics: </b></td>
			<td valign='top'> ".stripslashes($sql_row_values['topics'])."</td>
			</tr>		
			</table>";
		...
		$sql_get_ttl_mttl="SELECT * FROM capmus_users WHERE id='".$_POST['teacher_id']."'";
	...
	?>
	<input rows="10" cols="90" id='email_to_send_on_MONTH_START_REPORT' name='email_to_send_on_MONTH_START_REPORT' readonly="readonly" type='hidden' value="<?php echo $email_to_send_on_MONTH_START_REPORT; ?>"/>
	<input id='student_email' name='student_email' readonly="readonly" type='hidden' value="<?php echo $_POST['email_id']; ?>"/>	
	<input id='ttl_email' name='ttl_email' readonly="readonly" type='hidden' value="<?php echo $row_get_ttl_email['email']; ?>"/>
	<input id='mttl_email' name='mttl_email' readonly="readonly" type='hidden' value="<?php echo $row_get_mttl_email['email']; ?>"/>
...
<form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post">
	<?php if(($_SESSION['userType']==1 || $_SESSION['userType']==8 || $_SESSION['userType']==15) && $row['feedback']=='') { ?>
	<? echo "<td valign='top'>"?> 
	<input type='submit' value='Send Email' name='send_email_month_start_report' class="button" />
	<? echo "</td>"?>
	<? echo "<td valign='top'>"?> 
	<input type='email' value='' id='email_id' name='email_id' placeholder='Enter email' required/>
	<? echo "</td>"?>
	<? } ?>
