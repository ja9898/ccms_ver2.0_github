...
<div id="label">Team:</div><div id="field"><?php echo getDataList_leave_application_new_IT_version('','IT_id',17,$_SESSION['userId']);?><?php
getFilterSubmit();
?></div>
...
echo $seven_days_range = floor($seven_days_range/(60*60*24));
			...
			if($systemdate>=prepareDate($_POST['LeaveStartDate']) && $seven_days_range<7)
			{
				if($_POST['casual_leave']==1)
				{	
					$result_CL_dual=mysql_query("SELECT * FROM campus_empleave WHERE EmpID='".$_POST['IT_id']."'
					AND LeaveType=2");
...
//Used $_POST['IT_id'] instead of $_SESSION['userId'] as TL has to apply for leave on behalf of teacher
if(isset($_POST['IT_id'])){
$teacherID = $_POST['IT_id'];
$sql_employee_info=("SELECT * FROM capmus_users WHERE id='".$_POST['IT_id']."' ");
$sql_employee_info_result=mysql_query($sql_employee_info) or die(mysql_error());
$sql_employee_info_result=mysql_fetch_array($sql_employee_info_result);
