...
(function($) {
    ...
    $.fn.fixedTable = function(opts) {
        ...
        if (ft.length) { ft.width(maxWidth+20); }
        ...
        $("." + options.classColumn + " > .fixedTable", layout).height(h);
		$("." + options.classColumn + " > .fixedTable", layout).width(200);//make sure the row area of the fixed column matches the height of the main table, with the scrollbar
    ...
    function adjustSizes(options) {
        ...
        for (i = 0; i < widthArray.length; i++) {
            $("#" + Id + " .fixedContainer ." + options.classHeader + " table tr td").each(function(j) {
                if (j % ccount == i) {
                   // $(this).css("min-width", widthArray[i] + "px");
					 $(this).css("min-width", 100 + "px");
                }
            });
