...
//if($_SESSION['userId']==159 || $_SESSION['userId']==48 || $_SESSION['userId']==227 || $_SESSION['userId']==506)
//{
...
if (isset($_POST['submitted']) && $_POST['amount'] >=0 && !empty($_POST['date']) && $_POST['date']!='1970-01-01' && !empty($_POST['dateRecieved']) && $_POST['dateRecieved']!='1970-01-01') {

echo $_POST['method_old'];
echo "<br>";
echo $_POST['method_new'];
echo "<br>";
...
&& ($_POST['method_new']==2 || $_POST['method_new']==3 || $_POST['method_new']==4 ))
{
	...
	else
	{
		...
		//Checking Actual / SU Amount WITH Invoice / Original Amount
		if($_POST['amountDefaultNew'] != $_POST['amountOriginalNew'])
		{
			$statusPendRejAccpt = 0;
		}
		...
		$sql = "INSERT INTO `campus_transaction` ( `transactionID` ,  `date` ,  `studentID` , 
		`teacherID` , `schedule_id` ,  `courseID` ,  `method` , `method_array` , `currency_array` , 
		`amount_original` , `amount_gbp` , `amount` , `discount_tran` ,`comments` ,`operator`, 
		`classType` , `startTime` , `dateRecieved` , `LeadId` , `main_LeadId` , `sender_name` , 
		`email` , `cardSave_ccv_code` , `amount_usd_simple` , `VirtualTerminal_name` , 
		`VirtualTerminal_number` , `VirtualTerminal_date` , `datetime_now_accounts` , 
		...
		'{$_POST['amount_usd_simple']}' , '{$_POST['VirtualTerminal_name']}' , 
		...
		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	NEWLY ADDED
		$sql_discount = "UPDATE campus_schedule SET discount = '{$_POST['discount']}' ,
...
/* 				} */
		...
		//REMINDER RESET
		$sql_reminder = "UPDATE campus_schedule SET comments_reminder=NULL,date_reminder=NULL WHERE id='".$schedule."'";
		mysql_query($sql_reminder) or die(mysql_error());
		...
		/////////////////////////////////////////////////////// NEWLY ADDED // FOR USER LOG
		 $add_transaction="TranID:".nl2br( $_POST['transactionID']).",StartTime:".nl2br($sql_amount['startTime']).",Date_Recvd:".prepareDate( $_POST['date'])
						...
						user_log( $_SERVER['PHP_SELF'] , "ADD_TRANSACTION" , $_SESSION['userId'] ,$add_transaction, $_POST['comments']);
		 ///////////////////////////////////////////////////////
...
`classType` , `startTime` , `dateRecieved` , `LeadId` , `main_LeadId` , `sender_name` , 
`email` , `cardSave_ccv_code` , `amount_usd_simple` , `VirtualTerminal_name` , 
`VirtualTerminal_number` , `VirtualTerminal_date` , `datetime_now_accounts` , `bankNameId` ,
`amountDefaultNew` , `amountOriginalNew` , `feeDeductNew` , `totalReceivedNew` , `discountNew` ,
`amountDefaultNew_Usd` , `amountOriginalNew_Usd` , `feeDeductNew_Usd` , 
`totalReceivedNew_Usd` , `discountNew_Usd` , `statusPendRejAccpt`  ) 
VALUES(  '{$_POST['transactionID']}' ,  '".prepareDate($_POST['date'])."' , '{$_POST['studentID']}' , 
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	NEWLY ADDED
$sql_discount = "UPDATE campus_schedule SET 
`dues_amountDefaultNew` = '{$_POST['amountDefaultNew']}' , `dues_amountOriginalNew`='{$_POST['amountOriginalNew']}' , 
`dues_feeDeductNew`='{$_POST['feeDeductNew']}' , `dues_totalReceivedNew`='{$_POST['totalReceivedNew']}' , 
`dues_discountNew`='{$_POST['discountNew']}' ,
`dues_amountDefaultNew_Usd`='{$_POST['amountDefaultNew_Usd']}' , `dues_amountOriginalNew_Usd`='{$_POST['amountOriginalNew_Usd']}' , 
...
<div id="label">TransactionID:</div><div id="field"><input type='text' name='transactionID' id='transactionID' required/> </div>
...
<?php } else {   ?>
<div id="label">Date Received:</div><div id="field"><input type='text' name='date' value='<?php echo $systemdate; ?>' readonly='readonly'/> </div>
<?php } ?>
...
<div id="label">Recurring/Due Date:</div><div id="field"><input type='text' name='dateRecieved' value='<?=$current_year_for_recurr_date."-".$current_month_for_recurr_date."-".$paydate_date_for_recurr_date ?>' readonly='readonly' required/> </div>
