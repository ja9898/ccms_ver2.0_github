...
while($row_child = mysql_fetch_array($rs_child)){
	...
	if(trim($row_child['extId']) != ''){
		...
		$tofile1 .= "[66".$row_get_extid_number['extId']."]\r\n";
		$tofile1 .= "deny=0.0.0.0/0.0.0.0\r\n";
		$tofile1 .= "secret=BZH@ss6990\r\n";
		...
		if(trim($row_child['Phone1']) !=''){
				$tofile1 .= "dial=SIP/".trim($phone1_without_dashes_brackets_output)."@voip\r\n";
		...
		$tofile1 .= "mailbox=66".$row_get_extid_number['extId']."@device\r\n";
		...
		$tofile1 .= "callerid= <66".$row_get_extid_number['extId'].">\r\n";
	...
	$myFile = "voice-id-all-ids-with-numbers-PATTERN".date('dMYHis')."($from_date to $to_date).txt";
