...
//$sql_employee_info=("SELECT * FROM capmus_users WHERE LeadId='".$_SESSION['userId']."' ");
...
<div id="label">Team:</div><div id="field"><?php echo getDataList_leave_application_new_pa_version('','generalservices_id',14,$_SESSION['userId']);?><?php
getFilterSubmit();
?></div>
...
//////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////
if(isset($_POST['submitted']) && $_POST['teacherID_hidden']!='' && $_POST['teacherID_hidden']!=0)
{
	$_POST['sick_leave'];
	if(($_POST['sick_leave']!='' || $_POST['casual_leave']!='' || $_POST['other_leave']!='') && $_POST['LeaveStartDate']!='' && $_POST['LeaveEndDate']!='')
	{
		...
		$result_sick_overall_sum = leave_CheckSick_teamlead_version($JANUARY,$DECEMBER,$_POST['teacherID_hidden']);
		...
		else
		{
			if($systemdate<prepareDate($_POST['LeaveStartDate']))
			{
				...
				{$leave_type=1;if($SL_total_sum>4){				getMessages('error');
				...
				{$leave_type=3;}
				$sql_insert_leave = "INSERT INTO campus_empleave( EmpID , LeaveType , LeaveReason , LeaveStartDate , LeaveEndDate , NoOfDays , LeaveApplied , current_datetime)  VALUES('".$_POST['teacherID_hidden']."' , '".$leave_type."' , '{$_POST['LeaveReason']}' , '".prepareDate($_POST['LeaveStartDate'])."' , '".prepareDate($_POST['LeaveEndDate'])."' , '{$_POST['NoOfDays']}' , '".prepareDate($systemdate)."' , NOW() )";
...
echo $seven_days_range = floor($seven_days_range/(60*60*24));
			...
			if($systemdate>=prepareDate($_POST['LeaveStartDate']) && $seven_days_range<7)
			{
				if($_POST['casual_leave']==1)
				{	
					$result_CL_dual=mysql_query("SELECT * FROM campus_empleave WHERE EmpID='".$_POST['generalservices_id']."'
					AND LeaveType=2");
...
//Query of BASIC INFORMATION of the Employee on applying FILTER
//Used $_POST['generalservices_id'] instead of $_SESSION['userId'] as TL has to apply for leave on behalf of teacher
if(isset($_POST['generalservices_id'])){
$teacherID = $_POST['generalservices_id'];
$sql_employee_info=("SELECT * FROM capmus_users WHERE id='".$_POST['generalservices_id']."' ");
$sql_employee_info_result=mysql_query($sql_employee_info) or die(mysql_error());
$sql_employee_info_result=mysql_fetch_array($sql_employee_info_result);
...
$result_casual = leave_CheckCasual_teamlead_version($MONTH_START_DATE,$MONTH_END_DATE,$teacherID);
$rowcount_casual = mysql_num_rows($result_casual);

//Sick Leave function CALL and row count
$result_sick = leave_CheckSick_teamlead_version($JANUARY,$DECEMBER,$teacherID);
...
if(isset($_POST['generalservices_id'])){
echo "<label style='font-weight:bold'>Remaining Sick Leaves:".(4-$sick_total_count)."</label>";
...
<div id="label"></div><div id="field"><input type='submit' value='Submit' /><input type='hidden' value='1' name='submitted' /><input type='hidden' value=<?php echo $teacherID_hidden=$teacherID; ?> name='teacherID_hidden' /></div> 
