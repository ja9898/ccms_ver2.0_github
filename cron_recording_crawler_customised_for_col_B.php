...
function readCSV($csvFile){
	...
	while (!feof($file_handle) ) {
		...
		$source_caller_arr = explode('-',ltrim(trim($line_of_csv[4]),'SIP/'));
		$source_caller = $source_caller_arr[0];
		$source_caller = str_replace($str_search_arr, $str_replace_arr,$source_caller);
		
		//Destination
		$destination_caller_arr = explode('@',ltrim(trim($line_of_csv[7]),'SIP/'));
		$destination_caller = $destination_caller_arr[0];
		//$destination_caller = trim($line_of_csv[2]);
		
		$call_start_time = trim(date('Y-m-d H:i:s', strtotime($line_of_csv[8])));
		$call_end_time = trim(date('Y-m-d H:i:s', strtotime($line_of_csv[10])));
		
		$call_duration_in_seconds = trim($line_of_csv[11]);
		$call_status = trim($line_of_csv[13]);
		
		$call_date = trim(date('Y-m-d', strtotime($line_of_csv[8])));
...
$url = '65-254-45-122/8028.csv';
//$url = 'http://104.222.97.179/LOG/8020.csv';
$txt = file_get_contents("65-254-45-122/8028.csv");

$fp = fopen("upload_ref/8028.txt", "w");
fwrite($fp,$txt);
fclose($fp);
/*
$ch = curl_init ($url);
