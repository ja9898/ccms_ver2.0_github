...
//echo $_group=$_GET['group'];
$_paktime = $_GET['pakTime'];
...
while ($_teacherId=mysql_fetch_assoc($_resultt)) {
    ...
    echo "<pre>";
    $_sql="select distinct(teacherID) as teacherID, startTime, endTime, group_value from campus_timing where $_condition  and group_value=1 and teacherID=".$_teacherId['teacher'];
	$_result=mysql_query($_sql);
	
    while ($_rows=mysql_fetch_assoc($_result)) {
        $available=array();
		$stime = getData(nl2br( $_rows['startTime']),'time');
		$etime = getData(nl2br( $_rows['endTime']),'time');
		...
		*/
        //if ($available[$_LIST['time'][$pakTime[0]]]) {
			if($_paktime>=$stime || $_paktime<=$etime)
			{
