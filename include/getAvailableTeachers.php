...
$_classType=getClassTypeSchedule($_GET['classType']);
$_condition=getCondition($_GET['classType']);
...
while ($_teacherId=mysql_fetch_assoc($_resultt)) {
    ...
    echo "<pre>";
    $_sql="select distinct(teacherID) as teacherID, startTime, endTime from campus_timing where $_condition   and teacherID=".$_teacherId['teacher'];
        ...
        foreach($_LIST['time'] as $index=> $time ){
            if ($_rows['startTime']>$_rows['endTime']) {
                if ($_rows['startTime']> $index && $_rows['endTime']< $index && $index >0  ) {
                    ...
                    {
					 $_sql="select * from campus_schedule where std_status!=3 and std_status!=4 and std_status!=7 and (startTime<='".$time."' and ( endTime>'".$time."' or endTime='00:00')) and   $_classType and teacherID=".$_teacherId['teacher'];}
                    else{
                      $_sql="select * from campus_schedule where std_status!=3 and std_status!=4 and std_status!=7 and (startTime<='".$time."' and endTime>'".$time."') and   $_classType and teacherID=".$_teacherId['teacher'];
					}
