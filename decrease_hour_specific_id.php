...
*/
echo 	$result="SELECT campus_schedule.startTime,campus_schedule.endTime,campus_schedule.id as sch_id,campus_schedule.studentID,campus_schedule.std_status,campus_schedule.courseID,campus_schedule.teacherID,campus_schedule.classType,
		campus_students.id,campus_students.countryID 
		FROM campus_schedule 
		INNER JOIN campus_students 
		ON campus_schedule.studentID=campus_students.id and campus_schedule.status=1 and 
		(campus_schedule.std_status=1 OR campus_schedule.std_status=2) and 
		campus_schedule.teacherID IN (1959,1947,1935,1931,1720,1904,1675,1729,1945,1691,1928,1730,1686,1677,667,285,585,826,830,602,522,676,1943,1905,1793) 
		ORDER BY  campus_students.countryID asc";
echo "<br><br>first<br>";
//		1673,1677,1686,1783,1895,1902,1935,1720
	$rowcount = mysql_num_rows($result);
	$result=mysql_query($result);
	
while($row = mysql_fetch_array($result)){ 
$query2="select `campus_students`.id,`campus_students`.email,`campus_students`.mobile,`campus_students`.phone,`campus_students`.landline,`campus_students`.countryID from `campus_students` where campus_students.countryID=5 and campus_students.id=".$row['studentID'];
...
echo $update_time="UPDATE campus_schedule 
SET campus_schedule.endTime='".$time_var_endTime."' 
WHERE campus_schedule.studentID='".$rows2['id']."' and campus_schedule.courseID='".$row['courseID']."' and 
campus_schedule.teacherID='".$row['teacherID']."' and campus_schedule.classType='".$row['classType']."' and  
campus_schedule.id='".$row['sch_id']."' ";
