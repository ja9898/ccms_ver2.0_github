...
$FNAME  = preg_replace('/\s+/', '', $row['firstName']);
$LNAME  = preg_replace('/\s+/', '', $row['lastName']);
echo $update_uname_pass="UPDATE campus_students 
SET campus_students.username='".trim($FNAME)."_".trim($LNAME)."' , 
campus_students.password='".trim($FNAME).getextID(nl2br( $row['id']))."' , 
campus_students.user_type=4 
WHERE campus_students.id='".$row['id']."' ";
$out=mysql_query($update_uname_pass);
echo "<br>";
